...
function LogoBlock() {
  ...
  return (
    <div className="logo-block">
      {logoMissing ? <div className="logo-required">Official Bank of Botswana logo required</div> : <img src="/brand/bank-of-botswana-logo.png" alt="Bank of Botswana logo" onError={() => setLogoMissing(true)} />}
      <strong>BANK OF BOTSWANA</strong>
      <small>ASSIGNMENT WORKFLOW</small>
    </div>
  )
}
...
function LoginPage({ onSignedIn }) {
  ...
  return <main className="login-page"><section className="login-shell"><aside className="login-brand"><LogoBlock /><p className="brand-tagline">Secure. Reliable. Compliant.</p><p className="brand-subtext">Restricted demonstration environment for authorised review.</p></aside><form className="login-form" onSubmit={submit}><h1>Secure Demo Access</h1><p>Sign in to the Assignment Management System.</p><p className="muted">Access is restricted by assigned role. In this demo, use quick access buttons to preview the Manager and Officer workflows.</p><label>Email<input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required /></label><label>Password<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required /></label><button className="btn primary" type="submit">Sign in</button><div className="demo-buttons"><h4>Demo access</h4><button type="button" className="btn secondary demo-role manager-role" onClick={() => loginAs({ id: 'manager-001', name: 'Demo Manager', email: 'manager.demo@bankofbotswana.local', role: 'manager', position: 'Manager / Supervisor' })}><span>Continue as Manager</span><small>Preview manager workflow</small></button><button type="button" className="btn secondary demo-role officer-role" onClick={() => loginAs({ id: 'officer-001', name: 'Demo Officer', email: 'officer.demo@bankofbotswana.local', role: 'officer', position: 'Compliance Officer' })}><span>Continue as Officer</span><small>Preview officer workflow</small></button></div><small>Demo environment - role-based access is simulated for presentation. Production deployment would use database-backed authentication and server-side access control.</small></form></section></main>
}
...
function ManagerAssignments() {
  const [search, setSearch] = useState(''); const [officer, setOfficer] = useState('All'); const [status, setStatus] = useState('All'); const [priority, setPriority] = useState('All')
  const officers = getOfficers(); const assignments = getAssignments(); const { showToast } = useToast()
  const rows = assignments.filter((a) => { const own = officers.find((o) => o.id === a.assignedTo); const text = `${a.title} ${a.relatedEntity} ${own?.name || ''}`.toLowerCase(); return text.includes(search.toLowerCase()) && (officer === 'All' || a.assignedTo === officer) && (status === 'All' || a.status === status) && (priority === 'All' || a.priority === priority) })
  ...
  return <>
    ...
    <section className="kpi-grid small">{[
      ['All Assignments', assignments.length], ['Submitted', assignments.filter((a) => a.status === 'Submitted for Review').length], ['Overdue', assignments.filter((a) => a.status === 'Overdue').length], ['High Priority', assignments.filter((a) => a.priority === 'High' || a.priority === 'Critical').length],
    ].map(([k, v]) => <KpiCard key={k} icon={ClipboardList} label={k} value={v} hint="" tone="blue" />)}</section>
    <div className="filters"><SearchInput value={search} onChange={setSearch} placeholder="Search assignments, officers, entities..." /><label>Officer<select value={officer} onChange={(e) => setOfficer(e.target.value)}><option>All</option>{officers.map((o) => <option key={o.id} value={o.id}>{o.name}</option>)}</select></label><label>Status<select value={status} onChange={(e) => setStatus(e.target.value)}><option>All</option>{STATUS_OPTIONS.map((s) => <option key={s}>{s}</option>)}</select></label><label>Priority<select value={priority} onChange={(e) => setPriority(e.target.value)}><option>All</option>{PRIORITY_OPTIONS.map((s) => <option key={s}>{s}</option>)}</select></label></div>
    <Card title="Assignment Registry"><div className="table-wrap"><table><thead><tr><th>Assignment</th><th>Officer</th><th>Entity</th><th>Type</th><th>Status</th><th>Priority</th><th>Due Date</th><th>Latest Update</th><th>Actions</th></tr></thead><tbody>{rows.map((a) => <tr key={a.id}><td><strong>{a.title}</strong></td><td>{officers.find((o) => o.id === a.assignedTo)?.name || '-'}</td><td>{a.relatedEntity}</td><td>{a.assignmentType}</td><td><StatusBadge status={a.status} /></td><td><PriorityBadge priority={a.priority} /></td><td>{formatDate(a.dueDate)}</td><td>{a.progressNote}</td><td><details className="action-menu"><summary>Actions</summary><Link to={`/officer/assignments/${a.id}`}>View</Link><button onClick={() => setWorkflow(a, 'Under Review', 'Marked under review.')}>Mark Under Review</button><button onClick={() => setWorkflow(a, 'Approved', 'Assignment approved.')}>Approve</button><button onClick={() => setWorkflow(a, 'Returned for Corrections', 'Returned for corrections.')}>Return for Corrections</button></details></td></tr>)}</tbody></table></div></Card>
  </>
}
...
function ManagerSettings({ onReset }) {
  ...
  return <>
    ...
    <div className="settings-grid">
      <Card title="Demo Mode Settings"><p className="muted">Demo mode enabled with localStorage persistence.</p><ul className="mini-metrics"><li><span>Mode</span><strong>Enabled</strong></li><li><span>Data Variant</span><strong>Seeded Demo</strong></li><li><span>Persistence</span><strong>Local Browser Storage</strong></li></ul></Card>
      <Card title="User Management"><p className="muted">Current demo user and role footprint.</p><ul className="mini-metrics"><li><span>Total demo users</span><strong>{officers.length + 1}</strong></li><li><span>Roles</span><strong>Manager, Officer</strong></li><li><span>Manage users</span><strong><Link to="/manager/officers">Open Officers</Link></strong></li></ul></Card>
      <Card title="Status Workflow"><p className="muted">Submitted to In Review to Overdue to Approved to Completed</p><div className="workflow-pipeline"><span>Submitted</span><ChevronRight size={14} /><span>In Review</span><ChevronRight size={14} /><span>Overdue</span><ChevronRight size={14} /><span>Approved</span><ChevronRight size={14} /><span>Completed</span></div></Card>
      <Card title="Assignment Types"><p className="muted">Configured assignment templates and categories.</p><div className="chip-list">{['Compliance Review', 'Policy Update', 'Risk Assessment', 'Onsite Examination', 'Memo', 'Letter'].map((x) => <span key={x}>{x}</span>)}</div></Card>
      <Card title="Security Notice"><p className="muted">This demo uses simulated role-based sessions for presentation. Production deployment should use database-backed authentication, role-based authorization, audit logging, and internal server hosting.</p><ul className="mini-metrics"><li><span>Role-based Access</span><strong>Enabled</strong></li><li><span>Demo Data Only</span><strong>Enabled</strong></li><li><span>Audit Logging</span><strong>Simulated</strong></li><li><span>Session Isolation</span><strong>Client-side</strong></li></ul></Card>
      <Card title="Reset Demo Data"><p className="muted">Warning: this restores default officers, assignments, and update history.</p><ul className="mini-metrics"><li><span>Assignments in storage</span><strong>{assignments.length}</strong></li><li><span>Officers in storage</span><strong>{officers.length}</strong></li></ul><button className="btn secondary" onClick={() => setConfirm(true)}>Reset Demo Data</button></Card>
    </div>
